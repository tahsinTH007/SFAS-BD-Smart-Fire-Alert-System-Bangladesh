...
const SettingsPage = () => {
  ...
  const simulator = health?.dependencies.simulator;
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100">
      ...
      <main className="mx-auto flex max-w-4xl flex-col gap-5 px-4 py-6 sm:px-6">
        ...
        >
            ...
            <StatusRow
              ...
              detail={
                health && !health.dependencies.redis.enabled
                  ? "Disabled on this deployment — API works without it"
                  : "Optional — API works without it"
              }
              optional
            />
            <StatusRow
              ...
              detail={
                serial?.connected
                  ? `Arduino on ${serial.path} @ ${serial.baudRate}`
                  : feed.detail
              }
              optional
            />
          ...
          {simulator?.up && simulator.units.length > 0 && (
            <div className="mt-4 border-t border-slate-800 pt-3">
              <p className="mb-2 text-[11px] font-semibold uppercase tracking-wider text-slate-500">
                Simulated units
              </p>
              <ul className="grid gap-1.5 sm:grid-cols-2">
                {simulator.units.map((u) => (
                  <li
                    key={u.deviceCode}
                    className="flex items-center justify-between rounded-lg border border-slate-800 bg-slate-950/50 px-3 py-1.5 text-xs"
                  >
                    <span className="min-w-0 truncate">
                      <span className="font-mono text-slate-300">{u.deviceCode}</span>
                      {u.label && (
                        <span className="ml-1.5 text-slate-500">{u.label}</span>
                      )}
                    </span>
                    <span
                      className={
                        u.phase === "fire"
                          ...
                          : u.phase === "smoulder" || u.phase === "gas-leak"
              ...
              <p className="mt-2 text-[11px] leading-relaxed text-slate-500">
                No hardware is attached to this deployment. These units are
                generated by the API and pushed through the real ingest
                pipeline, so every alert you see was scored by the fusion engine.
              </p>
            </div>
          )}
